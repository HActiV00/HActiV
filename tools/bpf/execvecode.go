...
struct event_t {
    ...
    u32 puid;
    u32 pgid; //192 byte
    char comm[TASK_COMM_LEN]; //208
    char filename[100]; // 308
    char arg1[200];
    u32 namespaceinum;
};
...
for (int i = 1; i < 20 && offset < MAX_ARGS_SIZE - 1; i++) {
    ...
    offset += len - 1;  // 널 종료 문자 제외
    event.arg1[offset++] = ' ';  // 인자 사이에 공백 추가
}

if (offset > 0)
    event.arg1[offset - 1] = '\0';  // 마지막 공백을 널 종료 문자로 대체
else
    event.arg1[0] = '\0';  // 인자가 없는 경우 빈 문자열로 설정
